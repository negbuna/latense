...
class latense:
    ...
    def create_vector(self, task_name, dataset_name, split="train", num_samples=500, prompt_idx=0, layer_idx=-1):
        ...
        Args:
            task_name (str): Unique name to identify this vector (e.g., "math", "strategy_qa").
            dataset_name (str): HuggingFace dataset name.
            split (str): Dataset split to use (default: "train").
            num_samples (int): Number of examples to process.
            layer_idx (int): Layer to extract activations from.
        """
        ...
        for i in tqdm(indices, desc=f"Analyzing {task_name}"):
            ...
            # original_generation returns (answer, hidden_states_list, input_ids)
            ...
            if "strategy_qa" in dataset_name or "strategy-qa" in dataset_name or "trivia_qa" in dataset_name:
                ...
                # math verification using extract_answer and judge_answer
    ...
    def infer(self, prompt, steering_task=None, alpha=0.3, max_new_tokens=512, layer_idx=-1, ablation_mode="full"):
        ...
        Args:
            prompt (str): The input prompt (assumed to be already formatted with chat template).
            steering_task (str): The name of the steering vector to use.
            alpha (float): The steering strength.
            max_new_tokens (int): The maximum number of tokens to generate.
            layer_idx (int): The layer to apply steering to.
            ablation_mode (str): The ablation mode to use ('full', 'norm_only', or 'static').
        """
