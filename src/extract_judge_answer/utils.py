...
from eval_utils import extract_strategyqa_answer, judge_general_answer
...
def judge_answer(input, label, data_name="gsm8k", extract=True, prompt_idx=0):
    ...
    if "gsm8k" in data_name:
        ...
        if extract:
            ...
            label = str(label)
        return (input == label)
    elif "strategy-qa" in data_name:
        return judge_general_answer(input, label, data_name)
...
def extract_answer(text, data_name="gsm8k", prompt_idx=0, model_name="Qwen2.5-7B-Instruct"):
    ...
    if "gsm8k" in data_name:
        ...
        return extract_strategyqa_answer(text)
    else:
