...
def main(args):
    """
    Runs an alpha sensitivity analysis by applying steering at different alphas
    and logging the accuracy for each alpha.
    """
    ...
    results_file = os.path.join(results_dir, f"alpha_sweep_{model_name_safe}.jsonl")
    csv_results_file = os.path.join(results_dir, f"alpha_sweep_{model_name_safe}.csv")
    ...
    with open(csv_results_file, 'w', newline='') as csvfile:
        ...
        writer.writerow(["alpha", "accuracy", "correct_count", "total_count", "layer_idx", "ablation_mode", "avg_ppl", "avg_time_per_token", "avg_cos_sim", "avg_delta_h_norm", "avg_forward_time_us", "avg_steering_time_us", "avg_repetition_rate"])

    # --- Loop over alphas and evaluate ---
    alpha_range = [float(a.strip()) for a in args.alphas.split(',')]
    ...
    for alpha in alpha_range:
        ...
        for i in tqdm(range(num_samples), desc=f"Alpha {alpha} Eval"):
            example = dataset[i]
            ...
            log_data = {
                ...
                "avg_forward_time_us": avg_forward_time_us_token,
                "avg_steering_time_us": avg_steering_time_us_token,
                "repetition_rate": repetition_rate_token
            }
            
            # Token-level trace for Reasoning Dynamics plots (10% of samples)
            if i % max(1, num_samples // 10) == 0:
                log_data["token_cos_sims"] = gen_dict.get("cos_sims", [])
        ...
        print(f"Alpha {alpha} Accuracy: {accuracy:.4f} ({correct_count}/{total_count}) | PPL: {avg_ppl:.2f} | Time/Token: {avg_time:.4f}s | Rep Rate: {avg_rep_rate:.2%}")
        
        with open(csv_results_file, 'a', newline='') as csvfile:
            writer = csv.writer(csvfile)
            writer.writerow([alpha, accuracy, correct_count, total_count, args.layer_idx, args.ablation_mode, avg_ppl, avg_time, avg_cos_sim, avg_delta_h, avg_fwd_time, avg_steer_time, avg_rep_rate])
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Run Alpha Sensitivity Analysis for LaTense")
    ...
    # Alpha settings
    parser.add_argument("--alphas", type=str, default="0.1,0.3,0.5,0.7,1.0", help="Comma-separated list of alphas to test.")
    parser.add_argument("--layer_idx", type=int, default=31, help="Layer index to apply steering to.")
