...
def original_generation(input_text, model, tokenizer, device, layer_idx=-1):
    '''
    ...
    Args:
        ...
        tokenizer
        device
        layer_idx: layer index to extract hidden states from (default: -1)
    ...
    while True:
        ...
        if layer_idx == -1:
            ...
            # map layer_idx to tuple index (0 is embeddings, 1 is layer 0 output)
            idx_to_use = layer_idx + 1 if layer_idx >= 0 else layer_idx
            hidden_states = outputs.hidden_states[idx_to_use][:, -1]
